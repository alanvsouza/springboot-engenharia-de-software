...
public class FisicaResource {
    ...
    @Operation(description = "Cadastra uma pessoa física. Os dados são validados antes de serem salvos.")
    @ApiResponses(
        value = {
            @ApiResponse(responseCode = "200", description = "Pessoa física cadastrada!"),
            @ApiResponse(
                responseCode = "400",
                description = "Dados inválidos! O corpo da resposta traz o campo e o motivo de cada erro.",
                content = @Content
            )
        }
    )
    @PostMapping("/")
    public boolean saveFisica(@Valid @RequestBody FisicaDTO fisicaDto) {
    ...
    @PutMapping("/")
    public boolean update(@Valid @RequestBody FisicaDTO fisicaDto) {
